...
def test_writer_agent_write_report_success():
    ...
    assert results["markdown"] == "# Quantum Computing Future\n\n## Introduction\n\nThis is a report."
...
def test_writer_agent_invalid_json_retries_and_raises():
    """Verify write_report retries 3 times and raises error on persistent invalid JSON."""
    mock_ai_service = MagicMock(spec=AIService)
    mock_ai_service.generate_response.return_value = {
...
def test_writer_agent_invalid_research_inputs():
    ...
    # Missing research
    with pytest.raises(ValueError, match="Input research_output must contain a non-empty list of 'research'"):
