...
def test_planner_agent_create_plan_success():
    ...
    mock_ai_service.generate_response.return_value = {
        "response": '{\n  "topic": "Quantum Computing",\n  "difficulty": "Hard",\n  "estimated_sources": 8,\n  "sections": ["Intro", "Qubits", "Algorithms", "Future"],\n  "execution_order": ["Intro", "Qubits", "Algorithms", "Future"]\n}'
    }
    
    agent = PlannerAgent(ai_service_instance=mock_ai_service)
    plan = agent.create_plan(topic="Quantum Computing", style="Technical", depth="Standard")
...
def test_planner_agent_invalid_json_retries_and_raises():
    ...
    agent = PlannerAgent(ai_service_instance=mock_ai_service)
    with pytest.raises(ValueError, match="Failed to generate a valid research plan JSON"):
        agent.create_plan(topic="Quantum Computing", style="Technical", depth="Standard")
