...
import tempfile
import pytest
from backend.memory.memory_service import MemoryService, MemoryServiceInitError, MemoryServiceError
...
def test_memory_service_duplicate_id_error(temp_db_dir):
    ...
    with pytest.raises(ValueError, match="already exists"):
        service.add_document(document_id="doc1", text="different text", metadata={"key": "val"})
...
def test_memory_service_invalid_inputs(temp_db_dir):
    ...
    with pytest.raises(ValueError, match="must be str, int, float, or bool"):
        service.add_document(document_id="id1", text="text", metadata={"nested": {"a": 1}})
    with pytest.raises(ValueError, match="must be str, int, float, or bool"):
...
def test_memory_service_delete_document(temp_db_dir):
    ...
    service.add_document(document_id="doc_to_del", text="text block", metadata={"temp": True})
