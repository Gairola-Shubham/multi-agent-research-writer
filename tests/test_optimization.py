...
def test_lazy_initialization_ollama_client(mock_check_connection):
    """Verify OllamaClient does not run connection checks on creation."""
    client = OllamaClient(
        base_url="http://test-url", default_model="test-model", timeout=10
    )
...
def test_avoid_duplicate_startup_checks(
    mock_client, mock_remove, mock_open, mock_makedirs
):
    """Verify startup checks run logic exactly once and cache results."""
...
def test_health_endpoint_graceful_degradation(mock_chroma, mock_client):
    """Verify health endpoint handles errors gracefully without throwing 500."""
    mock_client.check_connection.side_effect = Exception(
        "Ollama connection failed exception"
    )
...
def test_health_endpoint_reuses_workflow_services(mock_workflow):
    """Verify health check endpoint reuses active services from workflow."""
