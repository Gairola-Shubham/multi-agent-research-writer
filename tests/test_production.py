...
def test_configuration_validation_empty_fields():
    """Verify that settings validation catches empty/blank strings in
    required fields.
    """
    with pytest.raises(ValidationError) as excinfo:
        Settings(LLM_MODEL=" ")
...
def test_health_endpoint(mock_memory, mock_search, mock_chroma, mock_ollama_client):
    """Verify that health endpoint queries and returns all required component
    statuses.
    """
    # Setup mocks
...
def test_startup_checks_success(
    mock_remove, mock_open, mock_makedirs, mock_ollama_client
):
...
def test_startup_checks_critical_chromadb_failure(mock_makedirs, mock_ollama_client):
    """Verify that startup checks raise RuntimeError when ChromaDB directory
    is not writable.
    """
