...
def test_research_agent_conduct_research_success(mock_memory_service_class):
    """Verify conduct_research works with valid JSON responses from AI,
    search, and memory.
    """
    ...
    mock_search_service.search.return_value = [
        {
            "title": "Quantum Computing Source",
            "url": "http://quantum.com",
            "snippet": "A guide to qubits.",
        }
    ]
    ...
    mock_memory_service_class.search.return_value = [
        {
            "id": "doc123",
            "text": "Historical context on superposition",
            "metadata": {"topic": "Quantum Computing", "section": "Intro"},
            "distance": 0.1,
        }
    ]
    ...
    plan = {"topic": "Quantum Computing", "sections": ["Intro"]}
    ...
    assert section_data["sources"] == [
        {"title": "Quantum Computing Source", "url": "http://quantum.com"}
    ]

    # Assert search was called
    mock_search_service.search.assert_called_once_with(
        query="Quantum Computing Intro", max_results=3
    )
    # Assert memory was searched
    mock_memory_service_class.search.assert_called_once_with(
        query="Quantum Computing Intro", top_k=3
    )
...
def test_research_agent_search_failure_fallback(mock_memory_service_class):
    ...
    mock_memory_service_class.search.return_value = [
        {
            "id": "doc1",
            "text": "Cached superposition data",
            "metadata": {},
            "distance": 0.2,
        }
    ]
...
def test_research_agent_memory_failure_fallback(mock_memory_service_class):
    """Verify ResearchAgent fallback works when MemoryService search fails
    but search succeeds.
    """
    mock_ai_service = MagicMock(spec=AIService)
    mock_ai_service.generate_response.return_value = {
    ...
    mock_search_service = MagicMock(spec=SearchService)
    mock_search_service.search.return_value = [
        {"title": "Web Source", "url": "http://web.com", "snippet": "Web context"}
    ]
...
def test_research_agent_duplicate_document_handling(mock_memory_service_class):
    """Verify that if saving to memory fails (e.g. duplicate ID or database error),
    ...
    mock_memory_service_class.add_document.side_effect = ValueError(
...
def test_research_agent_malformed_memory_results(mock_memory_service_class):
    """Verify that if memory results are malformed (e.g., metadata is None/missing),
    the agent handles it safely.
    """
    mock_ai_service = MagicMock(spec=AIService)
    mock_ai_service.generate_response.return_value = {
        "response": (
...
def test_research_agent_invalid_json_retries_and_raises(mock_memory_service_class):
    """Verify research_section retries 3 times and raises error on
    ...
    with pytest.raises(
        ValueError, match="Failed to generate valid research JSON for section 'Intro'"
    ):
        agent.conduct_research(plan)
...
def test_research_agent_invalid_plan_inputs():
    ...
    # Missing sections
    with pytest.raises(
        ValueError, match="Input plan must contain a non-empty list of 'sections'"
    ):
