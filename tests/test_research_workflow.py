...
def test_research_workflow_success():
    """Verify that the workflow executes successfully in the correct order
    with proper state updates.
    """
    ...
    mock_research.conduct_research.return_value = {
        "topic": "Test Topic",
        "research": [{"section": "Intro", "summary": "Info"}],
    }

    mock_writer = MagicMock()
    mock_writer.write_report.return_value = {
        "topic": "Test Topic",
        "title": "Test Title",
        "markdown": "# Test Title",
    }
    ...
    mock_reviewer.review_report.return_value = {
        "score": 90,
        "strengths": ["Clear"],
        "issues": [],
        "suggestions": [],
...
def test_research_workflow_node_failure():
    ...
    workflow = ResearchWorkflow(planner=mock_planner, research_agent=mock_research)
...
def test_research_workflow_public_run():
    ...
    with patch(
        "backend.workflows.research_workflow.ResearchWorkflow"
...
def test_workflow_agent_retry_success(mock_ai_generate):
    """Verify that the agent's internal retry mechanism works within
    workflow execution.
    """
    ...
    mock_ai_generate.side_effect = [
        ...
        ValueError("Timeout"),
        {
            "response": (
                ...
                '"estimated_sources": 5, "sections": ["Intro"], '
                '"execution_order": ["Intro"]}'
            )
        },
    ]
