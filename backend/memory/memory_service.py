...
class MemoryService:
    def __init__(self, db_path: Optional[str] = None, collection_name: str = "research_memory"):
        ...
        try:
            ...
            raise MemoryServiceInitError(f"Failed to create database directory: {e}") from e

        logger.info(f"Initializing MemoryService with db_path='{self.db_path}' and collection='{self.collection_name}'")
        try:
    ...
    def add_document(self, document_id: str, text: str, metadata: dict) -> None:
        ...
        # Validate metadata values (Chroma requires primitive types: str, int, float, bool)
        for k, v in metadata.items():
            if not isinstance(k, str):
                raise ValueError(f"Metadata key must be a string: {k}")
            if v is not None and not isinstance(v, (str, int, float, bool)):
                raise ValueError(f"Metadata value for key '{k}' must be str, int, float, or bool. Got {type(v)}.")
        ...
        try:
            # None metadata values are not supported by some ChromaDB queries, so we clean them
        ...
        try:
            ...
            if res and res.get("ids") and len(res["ids"][0]) > 0:
                for idx in range(len(res["ids"][0])):
                    results.append({
                        "id": res["ids"][0][idx],
                        "text": res["documents"][0][idx] if res.get("documents") else "",
                        "metadata": res["metadatas"][0][idx] if res.get("metadatas") else {},
                        "distance": res["distances"][0][idx] if res.get("distances") else 0.0
                    })
