...
# Try imports for optional document generation libraries to allow loading even if missing,
...
def format_inline_markdown(text: str) -> str:
    """
    Converts inline Markdown formatting (bold, italic, code) to ReportLab Paragraph HTML tags.
    """
                ...
                if p_text.startswith('# '):
                    ...
                    for item in items:
                        ...
                        if item.startswith('- ') or item.startswith('* '):
                            ...
                            flowables.append(Paragraph(f"&bull; {formatted_item}", styles['Bullet']))
                    flowables.append(Spacer(1, 6))
                    ...
                    for item in items:
                        ...
                        if match:
                            ...
                            formatted_item = format_inline_markdown(item_content)
                            flowables.append(Paragraph(f"{num}. {formatted_item}", styles['Normal']))
                    flowables.append(Spacer(1, 6))
                else:
...
def parse_markdown_to_docx(markdown_text: str, doc):
    """
    Parses Markdown text and appends corresponding paragraphs/headers/lists to a python-docx Document.
    """
    lines = markdown_text.splitlines()
    blocks = []
    in_code_block = False
...
def export_markdown(report: dict, output_path: str) -> str:
    ...
    try:
        ...
        logger.error(f"Export failed - format: markdown, path: {output_path}, error: {str(e)}")
        raise
...
def export_pdf(report: dict, output_path: str) -> str:
    ...
    try:
        ...
        # Modify existing styles in-place to avoid namespace collisions and ensure clean output
        ...
        # 1. Document Title
        flowables.append(Paragraph(report.get("title", "Research Report"), styles['Title']))
        ...
        review_table_data = [
            [Paragraph("<b>Report Evaluation & Metadata Summary</b>", metadata_title_style), ""],
            [Paragraph("Topic", label_style), Paragraph(report.get("topic", ""), val_style)],
            [Paragraph("Evaluation Score", label_style), Paragraph(f"<b>{score}/100</b>", val_style)],
            [Paragraph("Strengths", label_style), Paragraph("<br/>".join([f"&bull; {format_inline_markdown(s)}" for s in strengths]) if strengths else "None", val_style)],
            [Paragraph("Issues Identified", label_style), Paragraph("<br/>".join([f"&bull; {format_inline_markdown(i)}" for i in issues]) if issues else "None", val_style)],
            [Paragraph("Suggestions", label_style), Paragraph("<br/>".join([f"&bull; {format_inline_markdown(s)}" for s in suggestions]) if suggestions else "None", val_style)],
            [Paragraph("Changes Applied", label_style), Paragraph("<br/>".join([f"&bull; {format_inline_markdown(c)}" for c in changes_applied]) if changes_applied else "None", val_style)],
            [Paragraph("Generated Timestamp", label_style), Paragraph(timestamp, val_style)],
        ]
        ...
        content_flowables = parse_markdown_to_flowables(report.get("final_markdown", ""), styles)
        ...
        logger.error(f"Export failed - format: pdf, path: {output_path}, error: {str(e)}")
        raise
...
def export_docx(report: dict, output_path: str) -> str:
    ...
    try:
        ...
        logger.error(f"Export failed - format: docx, path: {output_path}, error: {str(e)}")
        raise
