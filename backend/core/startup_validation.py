...
def run_startup_checks(force: bool = False) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(test_file, "w", encoding="utf-8") as f:
                f.write("test")
        logger.info(
            "Export status - System temporary directory is verified and writable."
        )
    except Exception as e:
        logger.critical(
            f"Export status - System temporary directory is NOT writable: {e}"
        )
    ...
    try:
        ...
        if is_connected:
            ...
            logger.warning(
                f"Ollama status - server at {settings.OLLAMA_BASE_URL} is unreachable."
            )
    except Exception as e:
