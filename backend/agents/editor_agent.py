...
class EditorAgent:
    ...
    def edit_report(self, report_output: Dict[str, Any], review_output: Dict[str, Any]) -> Dict[str, Any]:
        """
        Edits and refines the report markdown based on reviewer suggestions and general readability.
        ...
        logger.info(f"EditorAgent starting refinement process for report title='{title}' under topic='{topic}'")

        prompt = (
            "You are a professional editor. Your task is to edit, refine, and polish the provided Markdown report based on peer review feedback, and output a structured response as a JSON object.\n"
            ...
            "- Strict constraint: Preserve all factual details; do not invent new facts.\n"
            "- Strict constraint: Retain standard Markdown formatting tags.\n\n"
            "The JSON object must strictly match this structure:\n"
            "{\n"
            f'  "title": "{title}",\n'
            '  "final_markdown": "The refined and improved Markdown report content...",\n'
            '  "changes_applied": ["First improvement/change description", "Second improvement/change description"]\n'
            "}\n\n"
            "Make sure that all double-quotes, newlines, and backslashes inside the JSON fields are correctly escaped.\n\n"
            f"Topic: {topic}\n"
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                logger.debug(f"EditorAgent calling AIService (attempt {attempt}/{max_attempts})")
                ...
                validated_report = self._validate_and_sanitize_edited_report(edited_json, topic, title)
                logger.info(f"EditorAgent successfully refined and validated report for topic='{topic}'")
                return validated_report

            except (json.JSONDecodeError, ValueError) as e:
                logger.warning(f"EditorAgent failed to parse/validate JSON on attempt {attempt}: {e}. Raw response: {raw_text}")
                if attempt == max_attempts:
                    logger.error(f"EditorAgent failed to generate a valid edited report after {max_attempts} attempts.")
                    raise ValueError(f"Failed to generate a valid editor JSON: {e}") from e
    ...
    def _validate_and_sanitize_edited_report(self, data: Any, expected_topic: str, expected_title: str) -> Dict[str, Any]:
