...
class PlannerAgent:
    ...
    def create_plan(self, topic: str, style: str, depth: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"PlannerAgent creating research plan for topic='{topic}', style='{style}', depth='{depth}'")
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                logger.debug(f"PlannerAgent calling AIService (attempt {attempt}/{max_attempts})")
                ...
                logger.info(f"PlannerAgent successfully created and validated plan for topic='{topic}'")
                return validated_plan
                
            except (json.JSONDecodeError, ValueError) as e:
                logger.warning(f"PlannerAgent failed to parse/validate JSON on attempt {attempt}: {e}. Raw response: {raw_text}")
                if attempt == max_attempts:
                    logger.error(f"PlannerAgent failed to generate a valid plan after {max_attempts} attempts.")
                    raise ValueError(f"Failed to generate a valid research plan JSON: {e}") from e
    ...
    def _validate_and_sanitize_plan(self, data: Any, expected_topic: str) -> Dict[str, Any]:
        if not isinstance(data, dict):
            raise ValueError("Response is not a JSON object (dictionary)")
            
        required_keys = ["topic", "difficulty", "estimated_sources", "sections", "execution_order"]
