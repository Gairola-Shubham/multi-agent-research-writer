...
class WriterAgent:
    ...
    def write_report(self, research_output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(research_data, list) or not research_data:
            raise ValueError("Input research_output must contain a non-empty list of 'research'")

        logger.info(f"WriterAgent starting report draft generation for topic='{topic}' across {len(research_data)} sections.")

        prompt = (
            "You are a professional technical writer and editor. Your task is to write a comprehensive, publication-ready research report in Markdown based on the provided research summaries.\n"
            ...
            '  "markdown": "The complete report content in Markdown format, following all instructions below."\n'
            "}\n\n"
            "Markdown instructions:\n"
            "- Start directly with the title as an H1 heading (e.g. # Title)\n"
            "- Include an Introduction section (H2: ## Introduction)\n"
            "- For each researched section, include an appropriate H2 heading (e.g. ## Section Name)\n"
            ...
            "Make sure that all double-quotes, newlines, and backslashes inside the 'markdown' field are correctly escaped as required for a JSON string value.\n\n"
            f"Topic: {topic}\n"
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                logger.debug(f"WriterAgent calling AIService (attempt {attempt}/{max_attempts})")
                ...
                validated_report = self._validate_and_sanitize_report(report_json, topic)
                logger.info(f"WriterAgent successfully generated and validated markdown report for topic='{topic}'")
                return validated_report

            except (json.JSONDecodeError, ValueError) as e:
                logger.warning(f"WriterAgent failed to parse/validate JSON on attempt {attempt}: {e}. Raw response: {raw_text}")
                if attempt == max_attempts:
                    logger.error(f"WriterAgent failed to generate a valid report after {max_attempts} attempts.")
                    raise ValueError(f"Failed to generate a valid report JSON: {e}") from e
    ...
    def _validate_and_sanitize_report(self, data: Any, expected_topic: str) -> Dict[str, Any]:
