...
class OllamaConnectionError(LLMConnectionError, OllamaError):
    """Raised when the Ollama server is unreachable."""

    pass


class OllamaTimeoutError(LLMTimeoutError, OllamaError):
    """Raised when a request to Ollama times out."""

    pass


class OllamaModelNotFoundError(LLMModelNotFoundError, OllamaError):
...
class OllamaProvider(BaseLLMProvider):
    def __init__(
        ...
        self._base_url = base_url or settings.OLLAMA_HOST or settings.OLLAMA_BASE_URL
    ...
    def base_url(self) -> str:
        """Get the base URL of the Ollama provider."""
        return self._base_url

    @property
    def default_model(self) -> str:
        """Get the default model name for the Ollama provider."""
        return self._default_model

    @property
    def is_online(self) -> bool:
        """Check if the Ollama provider is online (cached check)."""
    ...
    def is_online(self, value: bool) -> None:
        """Set the online status cache of the Ollama provider."""
        ...
        **kwargs,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Ensure requests doesn't fail on connection check with custom timeouts
